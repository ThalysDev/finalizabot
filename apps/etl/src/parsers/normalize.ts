...
export function normalizeShotsFromSofaScore(
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    const eventId = safeStr(
      o.id ?? o.incidentId ?? o.eventId ?? `${matchId}-${i}`,
    );
    ...
    const playerName =
      typeof o.player === "object" && o.player !== null
        ? safeStr(
            (o.player as Record<string, unknown>).name ??
              (o.player as Record<string, unknown>).shortName,
          ) || undefined
        : undefined;
    ...
    const coords =
      typeof o.playerCoordinates === "object" && o.playerCoordinates !== null
